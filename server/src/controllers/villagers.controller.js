...
export async function getVillager(req, res) {
  try {
    const villager = await villagersService.getVillager(req.params.code);
...
export async function updateVillager(req, res) {
  try {
    const result = await villagersService.updateVillager(req.params.code, req.body);
...
export async function deleteVillager(req, res) {
  try {
    const result = await villagersService.deleteVillager(req.params.code);
    if (!result) return res.status(404).json({ success: false, error: { message: "Villager not found" } });
