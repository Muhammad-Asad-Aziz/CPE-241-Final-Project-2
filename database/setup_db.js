...
function getTableCounts(queryFn) {
  const out = queryFn([
    ...
    "  (select count(*) from anvil_line_item);" // ONLY THE LAST ITEM IN THE LIST CAN HAVE THE SEMI COLON + SHOULDN'T HAVE A COMMA
...
function getSeedDecision(counts) {
  ...
  const missingCraftLessData = counts.transfer === 0 && counts.transferLineItem === 0 && counts.crafting === 0 && counts.craftingLineItem === 0 &&counts.anvil === 0 && counts.anvilLineItem === 0;
...
function logCounts(counts) {
  console.log(
    ...
    ", anvil=" + counts.anvil +
    ", anvil_line_item=" + counts.anvilLineItem /* LAST ITEM IN LIST MUST NOT HAVE "+" in the end */
